...
void load_settings_nvs(){
    ...
    if(utils_nvs_get_str(NVS_SERIAL_NUMBER, SerialNumber,100) == ESP_OK){
       ...
       if(utils_nvs_get_str(NVS_SERIAL_NUMBER, SerialNumber,100) == ESP_OK){
        utils_nvs_get_str(NVS_SERIAL_NUMBER, SerialNumber,100);
        // Check if SerialNumber starts with "NA-1507-"
        const char *prefix = "NA-1507-";
        ...
        if (strncmp(SerialNumber, prefix, prefix_len) == 0) {
         ...
         const char *prefix1 = "GVC-CA-";
        size_t prefix_len1 = strlen(prefix1);

        if (strncmp(SerialNumber, prefix1, prefix_len1) == 0) {
            // Shift string left to remove prefix
            memmove(SerialNumber, SerialNumber + prefix_len1, strlen(SerialNumber) - prefix_len1 + 1);
        }
